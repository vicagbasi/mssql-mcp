...
export function registerDataPatternTools(server: McpServer, connectionManager: ConnectionManager): void {
  ...
  registerMcpTool(
    ...
    async ({ connectionName, tableName, schema = "dbo", columnName, sampleSize = 1000 }): Promise<McpToolResponse> => {
      try {
        ...
        const actualSampleSize = clampSqlInteger(sampleSize, 1, 100000, 1000);
        ...
        for (const col of columns.slice(0, 10)) { // Limit to 10 columns for performance
          const columnIdentifier = quoteIdentifierPart(col);
          const columnNameLiteral = sqlStringLiteral(col);
          const analysis = await executeQuery(connection, `
            SELECT TOP 1
              ${columnNameLiteral} as column_name,
              COUNT(*) as total_rows,
              COUNT(DISTINCT ${columnIdentifier}) as distinct_values,
              COUNT(${columnIdentifier}) as non_null_count,
              COUNT(*) - COUNT(${columnIdentifier}) as null_count,
              CAST(COUNT(${columnIdentifier}) * 100.0 / COUNT(*) AS DECIMAL(5,2)) as non_null_percentage,
              CAST(COUNT(DISTINCT ${columnIdentifier}) * 100.0 / COUNT(${columnIdentifier}) AS DECIMAL(5,2)) as uniqueness_percentage
            FROM (SELECT TOP ${actualSampleSize} ${columnIdentifier} FROM ${tableRef}) sample
          `);

          const topValues = await executeQuery(connection, `
            SELECT TOP 10
              ${columnIdentifier} as value,
              COUNT(*) as frequency,
              CAST(COUNT(*) * 100.0 / (SELECT COUNT(*) FROM (SELECT TOP ${actualSampleSize} ${columnIdentifier} FROM ${tableRef} WHERE ${columnIdentifier} IS NOT NULL) s) AS DECIMAL(5,2)) as percentage
            FROM (SELECT TOP ${actualSampleSize} ${columnIdentifier} FROM ${tableRef} WHERE ${columnIdentifier} IS NOT NULL) sample
  ...
  registerMcpTool(
    ...
    async ({ connectionName, schema = "dbo", maxRows = 1000 }): Promise<McpToolResponse> => {
      try {
        const connection = await connectionManager.getConnection(connectionName);
        const schemaLiteral = sqlStringLiteral(schema);
        const actualMaxRows = clampSqlInteger(maxRows, 1, 1000000, 1000);
        ...
        const result = await executeQuery(connection, `
          ...
          SELECT
            ...
            CASE
              WHEN ts.row_count <= ${actualMaxRows}
          ...
          WHERE ts.row_count <= ${actualMaxRows * 2} -- Include some larger tables for analysis
          ORDER BY
            CASE
              WHEN CASE
                WHEN ts.row_count <= ${actualMaxRows}
  ...
  registerMcpTool(
    ...
    async ({ connectionName, schema = "dbo", minNullPercentage = 10 }): Promise<McpToolResponse> => {
      try {
        const connection = await connectionManager.getConnection(connectionName);
        const schemaLiteral = sqlStringLiteral(schema);
        const actualMinNullPercentage = clampSqlNumber(minNullPercentage, 0, 100, 10);
        ...
        const result = await executeQuery(connection, `
          ...
          HAVING CAST((p.rows - COUNT_BIG(CASE WHEN c.name IS NOT NULL THEN 1 END)) * 100.0 / NULLIF(p.rows, 0) AS DECIMAL(5,2)) >= ${actualMinNullPercentage}
